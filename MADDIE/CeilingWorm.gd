...
@export var TimeToCharge : float = 3
@export var LaunchSpeed : float = 3
var LaunchTarget : Vector2
@onready var floor_check: RayCast2D = $FloorCheck
var rearsitsuglylittleheadpoint : Vector2
@export var rearsitsuglylittleheadrange : Vector2
@export var RearSpeed : float
...
func StateChanged(_OldState : States,NewState : States):
	if(NewState!=States.Preying):
		...
		#Drop()
	if(NewState == States.Charging):
		Charge()
	if(NewState == States.Launching):
		Launch()
...
func Charge():
	rearsitsuglylittleheadpoint = Vector2(randf_range(-rearsitsuglylittleheadrange.x,rearsitsuglylittleheadrange.x),randf_range(-rearsitsuglylittleheadrange.y,rearsitsuglylittleheadrange.y))
	rearsitsuglylittleheadpoint += worm_graphic.points[0]
	await get_tree().create_timer(TimeToCharge).timeout
	CurrentState = States.Launching

func Launch():
	LaunchTarget = AggroTarget.global_position
...
func _ready() -> void:
	...
	hitbox.body_entered.connect(HitboxBodyEntered)
	wormseed = randi()
...
func _process(delta: float) -> void:
	elapsedtime += delta
	floor_check.global_position = to_global(worm_graphic.points[0])
	hitbox.global_position = to_global(worm_graphic.points[0])
	if(CurrentState==States.Chasing):
		if(floor_check.is_colliding()):
			...
			worm_graphic.points[0] = worm_graphic.points[0].move_toward(worm_graphic.points[0]+Vector2.DOWN,delta*ChaseSpeed)
	
	if(CurrentState == States.Charging):
		worm_graphic.points[0] = worm_graphic.points[0].move_toward(rearsitsuglylittleheadpoint,delta*RearSpeed)
		
	
	if(CurrentState==States.Launching):
		if(!floor_check.is_colliding()):
			worm_graphic.points[0] = worm_graphic.points[0].move_toward(worm_graphic.points[0].direction_to(to_local(LaunchTarget))*5000,delta*LaunchSpeed)
		else:
		...
		for i in range(1,worm_graphic.points.size()):
			if(worm_graphic.points[i].distance_to(worm_graphic.points[i-1])>SegmentLength):
				worm_graphic.points[i] = worm_graphic.points[i].move_toward(worm_graphic.points[i-1]+(Vector2.UP*(sin(wormseed+i+elapsedtime*ChasingWriggleSpeed)*ChasingOscillateMagnitude)),delta*LaunchSpeed)
		#if(LaunchTarget!=Vector2.ZERO):
	else:
		for i in range(1,worm_graphic.points.size()):
			if(worm_graphic.points[i].distance_to(worm_graphic.points[i-1])>SegmentLength):
...
func HitboxBodyEntered(body : Node2D):
	if(body.is_in_group("player")):
		if(body.machine.get_state()=="fall"):
			Health-=1
